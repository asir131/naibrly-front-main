...
const Analytics = () => {
  ...
  const {
    ...
    isError,
    error,
  } = useGetProviderAnalyticsQuery(undefined, {
    refetchOnFocus: true,
  ...
  } = useGetProviderServiceRequestsQuery(undefined, {
    refetchOnFocus: true,
    refetchOnReconnect: true,
  });
  const {
  ...
  } = useGetProviderReviewsQuery(undefined, {
    refetchOnFocus: true,
    refetchOnReconnect: true,
  });
  const {
    data: nearbyBundlesData,
    isLoading: nearbyBundlesLoading,
    isError: nearbyBundlesError,
    error: nearbyBundlesErrorData,
    refetch: refetchNearbyBundles,
  } = useGetProviderNearbyBundlesQuery(undefined, {
    refetchOnFocus: true,
    refetchOnReconnect: true,
  });
  const [updateServiceRequestStatus, { isLoading: isUpdatingRequest }] =
    useUpdateServiceRequestStatusMutation();
  const [updateBundleStatus, { isLoading: isUpdatingBundle }] =
    useUpdateBundleStatusMutation();
  ...
  const handleAccept = async (item, type) => {
    try {
      ...
      toast.success(
        type === "service" ? "Service request accepted" : "Bundle accepted",
      );
    } catch (error) {
  ...
  const handleCencelOrderConfirmSubmit = async (note) => {
    try {
      ...
      await Promise.all([refetchRequests(), refetchNearbyBundles()]);
      toast.success(
        selectedRequest ? "Service request cancelled" : "Bundle declined",
      );
      setOpen(false);
      if (selectedRequest?._id) {
        setHiddenRequestIds((prev) =>
          prev.includes(selectedRequest._id)
            ? prev
            : [...prev, selectedRequest._id],
        );
      }
      if (selectedBundle?._id) {
        setHiddenBundleIds((prev) =>
          prev.includes(selectedBundle._id)
            ? prev
            : [...prev, selectedBundle._id],
        );
      }
      setSelectedRequest(null);
      setSelectedBundle(null);
  ...
  const activeRequests = useMemo(
    () =>
      requestsData?.serviceRequests?.items?.filter(
        (req) =>
          req.status === "pending" && !hiddenRequestIds.includes(req._id),
      ) || [],
  ...
  const activeRequestTotalPages = Math.max(
    1,
    Math.ceil(activeRequests.length / activeRequestPageSize),
  );
  const activeBundleTotalPages = Math.max(
    1,
    Math.ceil(activeBundles.length / activeBundlePageSize),
  );
    ...
    <div className="analytics_layout md:px-[126px] md:py-[80px] max-sm:py-6 max-sm:px-6 flex flex-col gap-6">
          ...
          <div className="analytics_card w-full">
            <div className="flex  items-center justify-center gap-3">
              <span>
                ...
                >
                  <path
                    d="M1.23298 9.71048C1.14328 8.97052 1.09843 8.60054 1.14156 8.2979C1.26715 7.41649 1.88523 6.69451 2.7178 6.45667C3.00368 6.375 3.36526 6.375 4.08843 6.375H26.6616C27.3847 6.375 27.7463 6.375 28.0322 6.45667C28.8648 6.69451 29.4828 7.41649 29.6084 8.2979C29.6516 8.60054 29.6067 8.97052 29.517 9.71048C29.2781 11.6816 29.1586 12.6672 28.8659 13.4816C28.0185 15.839 26.082 17.6018 23.7085 18.1765C22.8884 18.375 21.9252 18.375 19.9989 18.375H10.7511C8.82477 18.375 7.86158 18.375 7.04154 18.1765C4.66797 17.6018 2.7315 15.839 1.88414 13.4816C1.59138 12.6672 1.47191 11.6816 1.23298 9.71048Z"
                    stroke="#0E7A60"
                    strokeWidth="2.25"
                  />
                  <path
                    d="M15.375 13.875H15.3885"
                    stroke="#0E7A60"
                    strokeWidth="3"
                    strokeLinecap="round"
                  ...
                  />
                  <path
                    d="M2.625 15.375L2.625 19.905C2.625 24.4871 2.625 26.7781 4.28473 28.2015C5.94446 29.625 8.61575 29.625 13.9583 29.625H16.7917C22.1343 29.625 24.8055 29.625 26.4653 28.2015C28.125 26.7781 28.125 24.4871 28.125 19.905V15.375"
                    stroke="#0E7A60"
                    strokeWidth="2.25"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                  <path
                    d="M21.375 6.375L21.2425 5.91141C20.5825 3.60134 20.2525 2.4463 19.4669 1.78565C18.6812 1.125 17.6376 1.125 15.5504 1.125H15.1996C13.1124 1.125 12.0688 1.125 11.2831 1.78565C10.4975 2.4463 10.1675 3.60134 9.50746 5.91141L9.375 6.375"
                    stroke="#0E7A60"
                    strokeWidth="2.25"
                  />
                </svg>
            ...
            <div className="flex justify-center items-center gap-6 w-full">
              <div className="flex items-center flex-col w-full border-r-[5px] border-[#0E7A60]">
                <h2 className="analytics_heading">
                  ...
                  {isLoading
                    ? "..."
                    : (analytics?.today?.earnings || 0).toFixed(2)}
                </h2>
                <h3 className="text-sm text-[#666]">Today</h3>
              </div>
              <div className="flex items-center flex-col w-full">
                <h2 className="analytics_heading">
                  ...
                  {isLoading
                    ? "..."
                    : (analytics?.month?.earnings || 0).toFixed(2)}
                </h2>
                <h3 className="text-sm text-[#666]">This Month</h3>
              </div>
            </div>
        ...
        {requestsLoading ? (
          ...
          pagedActiveRequests.map((request) => {
            ...
            return (
              <div
                key={request._id}
                className="relative w-full rounded-2xl bg-white shadow-sm ring-1 ring-black/5 p-6 md:p-7"
              >
                {/* status pill */}
                ...
                <div className="mb-5">
                  <p className="text-[20px] font-bold text-[#333]">
                    ...
                    <span className="text-[#0E7A60] text-sm font-medium">
                      /consult
                    </span>
                  </p>
                </div>
                ...
                <div className="mb-5 flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-center gap-2 text-[16px] text-[#F1C400]"></div>
                  </div>
                </div>
                ...
                <div className="mb-2 text-[15px]">
                  ...
                  <span className="text-[#7F7F7F]">
                    ...
                    {request.customer?.address?.aptSuite &&
                      `, ${request.customer.address.aptSuite}`}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex w-full justify-end gap-3">
                    <button
                      onClick={() =>
                        handleCencelOrderConfirm(request, "service")
                      }
                      type="button"
        ...
        {activeRequests.length > 0 && activeRequestTotalPages > 1 && (
          <div className="flex items-center justify-center gap-3 pt-2">
            <button
              type="button"
              onClick={() =>
                setActiveRequestPage((prev) => Math.max(prev - 1, 1))
              }
            ...
            <button
              type="button"
              onClick={() =>
                setActiveRequestPage((prev) =>
                  Math.min(prev + 1, activeRequestTotalPages),
                )
              }
              className="page_number"
      ...
      <div className="flex flex-col gap-[18px] w-full">
        ...
        {nearbyBundlesLoading ? (
          ...
          <div className="w-full p-4 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm flex items-center justify-between gap-3">
            <span>
              {nearbyBundlesErrorData?.status === 401
                ...
                : nearbyBundlesErrorData?.data?.message ||
                  "Failed to load bundle requests. Please try again."}
            </span>
            <button
              type="button"
          ...
          <div className="flex flex-col md:flex-row items-center justify-between w-full gap-[10px]">
            {pagedActiveBundles.map((bundle) => (
              <div key={bundle._id} className="w-full">
                <BundleRequestCard
                  bundle={bundle}
                  handleCencelOrderConfirm={() =>
                    handleCencelOrderConfirm(bundle, "bundle")
                  }
          ...
          <div className="relative w-full rounded-2xl bg-white shadow-sm ring-1 ring-black/5 p-6 md:p-7">
            <p className="text-center text-[#666]">
              No active bundle requests at the moment.
            </p>
          </div>
        )}
        {activeBundles.length > 0 && activeBundleTotalPages > 1 && (
          <div className="flex items-center justify-center gap-3 pt-2">
            <button
              type="button"
              onClick={() =>
                setActiveBundlePage((prev) => Math.max(prev - 1, 1))
              }
              className="page_number"
            ...
            <button
              type="button"
              onClick={() =>
                setActiveBundlePage((prev) =>
                  Math.min(prev + 1, activeBundleTotalPages),
                )
              }
              className="page_number"
